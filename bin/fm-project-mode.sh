...
#   - <name> [<mode> forge=bitbucket +yolo] - <desc> (added <date>)  -> forge=bitbucket
#
# forge (orthogonal, bracket token forge=github|bitbucket) names the git forge
# a project's PR/MR lifecycle runs against. Absent means forge=github, so
# every existing bracket-less or bracket-present legacy entry is unaffected.
# Read it with --forge (below); the default two-word stdout contract is
# unchanged so existing mechanical consumers need no changes.
#
# Registered modes:
...
#
# --forge prints only the resolved forge (github|bitbucket) instead of the
# "<mode> <yolo>" pair, mirroring --raw's shape rather than changing the
# default stdout contract.
#
# An unknown/missing project or unknown mode falls back to "no-mistakes off" and warns
# to stderr, so a typo never silently drops the gate. An unknown forge token
# falls back to github with the same warning discipline.
...
parsed=$(awk -v n="$NAME" '
  $1=="-" && $2==n {
    mode="no-mistakes"; yolo="off"; forge="github";
    if ($3 ~ /^\[/) {
      s="";
      ...
      for (j=1; j<=k; j++) {
        if (a[j]=="+yolo") yolo="on";
        else if (a[j] ~ /^forge=/) { forge=a[j]; sub(/^forge=/, "", forge); }
      }
    }
